...
6. A selector's `&` obligation rides `Requires` (section 4.4): `specificity` and `Stylesheet`'s pair-form appends take bare `Selector`, so an unresolved `&` in either position is a compile error. Two mirrors stay runtime, because covariance cannot enforce presence and a node-form append erases the selector's parameter: `StyleRule.make` — the binder — requires every style rule reachable in its block through media transparency to reference `&` (CSS would silently prepend a descendant `&`; the model rejects the shape rather than rewriting the authored selector), and `Stylesheet.make`/`append` reject a node whose selector still needs a parent.
...
- **Selector nesting (`&`)**: shipped native-nested (`docs/selector-nesting.md`, driven by ok-apca's hue blocks in `docs/ok-apca-feedback.md`) — the complex-selector grammar, the `Requires` channel on `Selector`, the binder and root invariants, and nested rendering. The design's phase 3 stays open, each piece on consumer demand: container-level `Requires` parameters (`RuleSet`/`StyleRule`/`MediaRule`), `MediaRule` in the node union (authored top-level `@media`), relative `:has()` arguments, and flattened rendering via `under`.
