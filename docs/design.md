...
3. **One model, many projections.** Serialize flat CSS; serialize nested CSS; solve value expressions against bindings; compute selector specificity; merge stylesheets. The thing you verified is the thing you shipped.
...
src/stylesheet/             -> fashionable/stylesheet   [shipped]  Stylesheet<Refs>, render
```
...
Placement notes: `calc` names the CSS construct and gives the type its natural name (`Calc<Refs>`); `color` is separate because a color is not solvable to a number and will grow independently; `query/` is the home for the condition grammars — `MediaQuery` today, container and supports queries as sibling types when a consumer arrives (they share the module because they share the "prelude condition" role, not a grammar); `declaration` is its own subpath because it is the seam where the two language halves meet; the declaration-block at-rules get one module each (`fontFace/`, `property/`) so the at-rule family scales — `@layer` and friends become sibling modules rather than crowding a shared folder, with the shared block-text helpers in `internal/render.ts`. There is deliberately no separate render module: every renderable type carries its own `render` (section 7), the shared nested-form block renderer lives in `rule.internal.ts`, and the whole-sheet projections live on `Stylesheet.render` — the one module already downstream of every model module.
...
Documented v1 constraint: consume acos-carrying subtrees with `sin`/`cos`; an angle-typed expression that escapes to the top level serializes as angle-typed CSS (valid only where an angle is accepted).
...
`Calc<Refs>`/`Color<Refs>` phantom parameters carry unbound reference names; the runtime set is the source of truth. The containers (`Declaration<Refs>` through `Stylesheet<Refs>`) keep threading the union: an unbound ref serializes as `var(--name)`, so `Stylesheet<Refs>` reads as "this sheet reads these custom properties" — a dependency report, not an error state. This is what lets consumer 1 turn scheme-contract discovery into an assertion over the merged sheet. Escape hatch: the parameters default to `string`; if variadic inference over heterogeneous members proves noisy in practice, containers can relax to runtime-only ref sets without changing the runtime API.
...
```ts
interface Declaration<Refs> {
  name: string
  value: string | Calc<Refs> | Color<Refs> // Declaration.Value<Refs>
}
...
;(Declaration.refs, Declaration.equals, Declaration.isDeclaration)
```
...
```ts
type RuleSet.Member<Refs> = Declaration<Refs> | StyleRule<Refs> | MediaRule<Refs>
interface RuleSet<Refs>   { members: ReadonlyArray<Member<Refs>> }   // ordered, never sorted
interface StyleRule<Refs> { selector: Selector; block: RuleSet<Refs> }
interface MediaRule<Refs> { query: MediaQuery; block: RuleSet<Refs> }
...
4. `MediaRule` is not a `Stylesheet.Node` arm in v1 — media enters the model nested inside a style rule (`:root { @media ... { ... } }`), and the flat renderer distributes the prelude to emit the traditional `@media { :root { ... } }` blocks. This kills the third grammar distinction (top-level `@media` takes a rule list, not declarations) without a second media type. If a consumer needs authored top-level grouping of multiple selectors under one `@media`, the honest extension is a distinct rules-only top-level type.
...
Every renderable type carries its own `render`, so fragments and partials emit without ceremony; the whole-sheet projections live on `Stylesheet.render`. Flat versus nested output is a consumer choice, not a model property. Everything is deterministic: canonical formatting, stable ordering.
...
Stylesheet.RenderOptions = { format?: 'flat' | 'nested'; indent?: string; precision?: Precision; mediaSyntax?: 'prefix' | 'range' }
Stylesheet.render(sheet, options?): string
```

**The render-options family**: the `RenderOptions` interfaces extend one another, rooted at `MediaQuery.RenderOptions` — `Declaration.RenderOptions` adds `precision`, `RuleSet.RenderOptions` adds `indent` (`StyleRule`/`MediaRule` take it unchanged), `FontFaceRule`/`PropertyRule` extend the declaration tier with `indent`, and `Stylesheet.RenderOptions` tops the family with `format`. A key means the same thing wherever it appears, renderers ignore inherited keys that don't apply (a declaration has no media text to spell), and an options object built for a bigger renderer is accepted by every smaller one — build one `Stylesheet.RenderOptions` value and pass it to any `render` in the library. This is why `MediaQuery.render`'s option is named `mediaSyntax` rather than a bare `syntax`.

The default `'flat'` format un-nests by distributing preludes: walking a style rule it carries `(selector, accumulated MediaQuery)`; nested media queries and-compose (canonical ordering makes the composition deterministic); each maximal run of consecutive declarations emits one flat block, in member order — so flat and nested output obey the same cascade. The `'nested'` format emits each style rule as `StyleRule.render` does. Nested style-rule selector composition (`&`) is a later extension; v1 consumers do not nest style rules under style rules, and **every renderer throws** on one — silently emitting CSS whose selector semantics the model has not defined would be worse than refusing.

Emission conventions, pinned by golden fixtures shaped like consumer 1's real output: top-level sections join with one blank line, no trailing newline (consumer 1 adds its own at the file boundary); empty blocks emit nothing — a rule whose block is empty, or holds only empty media rules, is skipped, and the fragment renderers return the empty string for the same shapes; a media-wrapped flat block indents the selector one level and its declarations two (consumer 1's `@media { :root { ... } }` shape, byte-exact at `indent: '  '`).

Options change text, never meaning: `format`, `indent` (default tab), `precision` (the serialization context default), `mediaSyntax` (section 5.2).
...
2. **declaration + rule** — shipped. Refs threading through variadic `make` (type tests), member-order preservation and order-sensitive equality, non-member rejection; the section-6 type tests that involve at-rules and stylesheets land with phases 3-4.
