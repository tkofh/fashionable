...
This document is the repo-resident successor of the design brief that started the project (dtcg-resolver's `docs/ideas/css-library.md`). It records the whole v1 design, now fully shipped (section 8); what remains for v1 is the consumer migrations, in the consumer repos. Satellite documents carry the derivations and the in-flight designs: `docs/result-calc.md` (the `Calc<Vars, Result, Requires>` repartition — folded in, kept as the derivation record), `docs/feedback-dtcg-resolver.md` (consumer-1 field notes; shipped rounds are cleared from it), `docs/ok-apca-feedback.md` (consumer-2 field notes, the nesting motivation), `docs/vars.md` (the `Var` identity channel — folded in, kept as the derivation record), and `docs/selector-nesting.md` (the contract for the designed-but-unshipped `&` extension, folding back in here when its implementation lands).
...
- **Compound selector parts**: stable sort by `(kind rank, rendered text)`, ranked type/universal, id, class, pseudo-class, attribute, `:not()`, pseudo-element, enforced in constructors. Any fixed order renders a semantically identical compound; simple pseudo-classes precede attribute qualifiers and negations so the root-scoped house shapes render in consumer 1's exact current spelling (`:root[data-scheme='dark']`, `:root:not([data-scheme='light'])`), keeping the migration byte-diffable. Duplicates are kept (`.a.a` legally has specificity (0,2,0)); the grammar's own constraints are enforced (at most one type/universal part, at most one pseudo-element).
...
**The polarity convention.** The library carries two kinds of type-level brand, and their encodings invert each other. A **capability** (`ColorSpace`'s `Polar`) accumulates by intersection with an `unknown` default, and a position gates on _presence_. A **requirement** (`Calc`'s `Requires`; the selector layer's `Requires`, designed but unshipped, per `docs/selector-nesting.md`) accumulates by union with a `never` default, and a position gates on _absence_: a slot typed `X<R>` admits exactly the values whose requirements are a subset of `R`, so requirements restrict placement and discharge composes by `Exclude`. Covariance can enforce presence of a capability and absence of a requirement, and neither converse — the encoding is chosen by which direction needs compile-time teeth.
...
A `Selector` is a compound selector: a canonically ordered, non-empty list of simple-selector parts. Part constructors return one-part selectors; `and` merges. Combinators (descendant/child) are a later extension, not v1; the extension's design (`docs/selector-nesting.md`) widens `Selector` itself to the complex-selector grammar rather than layering a separate `Complex` type above the compound.
...
Values stay number-land; consumers apply units at the declaration boundary (calc-tree's posture). The API must not foreclose typed units later. Revisit against consumer 1's unit-folding experience (section 9).
...
Two union types and one phantom parameter make the illegal nesting shapes unrepresentable, so construction validates nothing at runtime:
...
4. `MediaRule` is not a `Stylesheet.Node` arm in v1 — media enters the model nested inside a style rule (`:root { @media ... { ... } }`) and renders there, nested. This kills the third grammar distinction (top-level `@media` takes a rule list, not declarations) without a second media type. Two extensions are deferred until a consumer needs them: the traditional flat `@media { selector { ... } }` shape (a render-time hoisting transform, section 7), and authored top-level grouping of multiple selectors under one `@media` (first sketched as a distinct rules-only top-level type; the nesting design instead slots `MediaRule` into the node union gated on `Requires = never` — `docs/selector-nesting.md` section 1).
...
- **Selector nesting (`&`)**: designed, unshipped (`docs/selector-nesting.md`, driven by ok-apca's hue blocks in `docs/ok-apca-feedback.md`). Until it ships, renderers throw on nested style rules (section 7), and ok-apca covers the gap by string-assembling that one selector.
...
- **Typed units**: values stay number-land with units at the declaration boundary; revisit after consumer 1 migrates. The read side has a designed route: typed `Var` constructors (`docs/vars.md` section 7) put a Result family on the handle, so a declared `<length>` var lifts as a length-result read with no new arithmetic machinery.
